...
window.MODULE_M0 = {
  ...
  lessons: [
    ...
    {
      id: "m0-l4", title: "Tester son code avec printf", type: "concept",
      content: {
        blocks: [
          { type: "text", value: "A 42 tu rends juste la fonction. Mais comment savoir si elle marche ? Tu ajoutes un main() de test qui appelle ta fonction et affiche le resultat avec printf." },
          { type: "text", value: "printf c'est ta loupe de debug. Il affiche ce que tu veux dans le terminal. Il est dans #include <stdio.h>." },
          { type: "code", value: "printf(\"%d\\n\", 42);          // Affiche un int : 42\nprintf(\"%c\\n\", 'A');         // Affiche un char : A\nprintf(\"%s\\n\", \"Hello\");     // Affiche une string : Hello\nprintf(\"%d\\n\", ft_strlen(\"abc\")); // Affiche le retour de ta fonction : 3\n\n// Le %d c'est pour les int.\n// Le %c c'est pour les char.\n// Le %s c'est pour les strings (char *).\n// Le \\n c'est le retour a la ligne." },
          { type: "text", value: "Le workflow complet pour tester une fonction :" },
          { type: "code", value: "#include <stdio.h>\n\nint ft_strlen(char *str)\n{\n    int i;\n    i = 0;\n    while (str[i])\n        i++;\n    return (i);\n}\n\n// --- Test (a retirer avant de rendre) ---\nint main(void)\n{\n    printf(\"%d\\n\", ft_strlen(\"Hello\"));   // Attendu : 5\n    printf(\"%d\\n\", ft_strlen(\"\"));        // Attendu : 0\n    printf(\"%d\\n\", ft_strlen(\"42 abc\"));  // Attendu : 6\n    return (0);\n}" },
          { type: "text", value: "Compile et execute :" },
          { type: "code", value: "gcc -Wall -Wextra -Werror ft_strlen.c -o test\n./test" },
          { type: "text", value: "Si les resultats correspondent a ce que tu attends, ta fonction marche. Sinon, ajoute des printf DANS ta fonction pour voir ce qui se passe a chaque etape :" },
          { type: "code", value: "int ft_strlen(char *str)\n{\n    int i;\n    i = 0;\n    while (str[i])\n    {\n        printf(\"i = %d, char = %c\\n\", i, str[i]);  // Debug\n        i++;\n    }\n    printf(\"total = %d\\n\", i);  // Debug\n    return (i);\n}" },
          { type: "key", value: "printf c'est pour TESTER et DEBUGGER. write c'est pour les fonctions qu'on REND a 42. Avant de rendre, retire tous les printf et le main." },
          { type: "text", value: "Autre exemple avec une fonction void qui affiche avec write :" },
          { type: "code", value: "#include <unistd.h>\n#include <stdio.h>\n\nvoid ft_putstr(char *str)\n{\n    int i;\n    i = 0;\n    while (str[i])\n    {\n        write(1, &str[i], 1);\n        i++;\n    }\n}\n\n// --- Test ---\nint main(void)\n{\n    ft_putstr(\"Hello\");\n    printf(\"\\n\");           // Juste pour le retour a la ligne du test\n    ft_putstr(\"\");          // Test string vide\n    printf(\"\\n\");\n    ft_putstr(\"42\");\n    printf(\"\\n\");\n    return (0);\n}" },
          { type: "text", value: "Pour les fonctions void qui affichent, le main les appelle simplement. Tu verifies visuellement que la sortie est correcte." },
        ],
        quiz: {
          question: "Tu veux afficher le resultat de ft_strlen(\"abc\") dans ton main de test. Quelle ligne ?",
          options: ["printf(\"%d\\n\", ft_strlen(\"abc\"));", "write(1, ft_strlen(\"abc\"), 1);", "printf(\"%s\\n\", ft_strlen(\"abc\"));", "printf(\"%c\\n\", ft_strlen(\"abc\"));"],
          correct: 0,
          explanation: "ft_strlen retourne un int, donc on utilise %d. %s c'est pour les strings, %c pour un seul caractere.",
        },
      },
    },
    {
