  ...
  lessons: [
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strcmp", code: "int ft_strcmp(char *s1, char *s2)\n{\n    int i;\n\n    i = 0;\n    while (s1[i] && s2[i] && s1[i] == s2[i])\n        i++;\n    return (s1[i] - s2[i]);\n}", explanation: "On avance tant que les chars sont identiques et non-nuls. À la sortie, la différence donne : 0 si identiques, négatif si s1 < s2, positif si s1 > s2." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strncmp", code: "int ft_strncmp(char *s1, char *s2, unsigned int n)\n{\n    unsigned int i;\n\n    if (n == 0)\n        return (0);\n    i = 0;\n    while (s1[i] && s2[i] && s1[i] == s2[i] && i < n - 1)\n        i++;\n    return (s1[i] - s2[i]);\n}", explanation: "Si n == 0 → 0. Sinon, on compare comme strcmp mais on s'arrête à n - 1 (car on compare aussi le char à la position n-1). Si les n premiers chars sont identiques → 0." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strcat", code: "char *ft_strcat(char *dest, char *src)\n{\n    int i;\n    int j;\n\n    i = 0;\n    while (dest[i])\n        i++;\n    j = 0;\n    while (src[j])\n    {\n        dest[i] = src[j];\n        i++;\n        j++;\n    }\n    dest[i] = '\\0';\n    return (dest);\n}", explanation: "On va au \\0 de dest, on copie src char par char, on termine avec un nouveau \\0. Le \\0 original de dest est écrasé par le premier char de src." },
          ...
          { type: "reveal", label: "Voir la solution ft_strncat", code: "char *ft_strncat(char *dest, char *src, unsigned int nb)\n{\n    unsigned int i;\n    unsigned int j;\n\n    i = 0;\n    while (dest[i])\n        i++;\n    j = 0;\n    while (src[j] && j < nb)\n    {\n        dest[i] = src[j];\n        i++;\n        j++;\n    }\n    dest[i] = '\\0';\n    return (dest);\n}", explanation: "Comme ft_strcat mais on copie au max nb chars de src. Le \\0 est TOUJOURS ajouté (contrairement à strncpy)." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strstr", code: "char *ft_strstr(char *str, char *to_find)\n{\n    int i;\n    int j;\n\n    if (to_find[0] == '\\0')\n        return (str);\n    i = 0;\n    while (str[i])\n    {\n        j = 0;\n        while (str[i + j] && to_find[j]\n               && str[i + j] == to_find[j])\n            j++;\n        if (to_find[j] == '\\0')\n            return (&str[i]);\n        i++;\n    }\n    return (0);\n}", explanation: "Pour chaque position i dans str, on vérifie si to_find commence là. L'astuce str[i + j] permet de comparer sans modifier i. Si on parcourt tout to_find (to_find[j] == '\\0'), c'est trouvé." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strlcat", code: "unsigned int ft_strlcat(char *dest, char *src, unsigned int size)\n{\n    unsigned int d_len;\n    unsigned int s_len;\n    unsigned int i;\n\n    d_len = 0;\n    while (dest[d_len] && d_len < size)\n        d_len++;\n    s_len = 0;\n    while (src[s_len])\n        s_len++;\n    if (d_len >= size)\n        return (size + s_len);\n    i = 0;\n    while (src[i] && d_len + i < size - 1)\n    {\n        dest[d_len + i] = src[i];\n        i++;\n    }\n    dest[d_len + i] = '\\0';\n    return (d_len + s_len);\n}", explanation: "On mesure dest (sans dépasser size), on mesure src. Si dest remplit déjà le buffer → retour spécial. Sinon on concatène avec limite et on retourne la longueur théorique." },
