    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strstr", code: "#include <stdio.h>\n\nchar *ft_strstr(char *str, char *to_find)\n{\n    int i;                     // Index dans str (position de depart)\n    int j;                     // Index dans to_find (progression du match)\n\n    if (to_find[0] == '\\0')   // Si to_find est vide, retourne str\n        return (str);\n    i = 0;                     // Commence au debut de str\n    while (str[i])             // Teste chaque position comme debut de match\n    {\n        j = 0;                 // Reinitialise l'index de to_find\n        while (str[i + j] && to_find[j] && str[i + j] == to_find[j])\n            j++;               // Les chars matchent, avance\n        if (to_find[j] == '\\0') // Match complet\n            return (&str[i]);  // Retourne pointeur vers le debut du match\n        i++;                   // Pas de match, essaie la position suivante\n    }\n    return (0);                // Aucun match, retourne NULL\n}\n\n// --- Test (a retirer avant de rendre) ---\nint main(void)\n{\n    char *r = ft_strstr(\"Hello World\", \"World\");\n    printf(\"%s\\n\", r);\n    return (0);\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strstr.c -o test && ./test", expectedOutput: "World\\n(null)", explanation: "Pour chaque position i dans str, on vérifie si to_find commence là. L'astuce str[i + j] permet de comparer sans modifier i. Si on parcourt tout to_find (to_find[j] == '\\0'), c'est trouvé." },
