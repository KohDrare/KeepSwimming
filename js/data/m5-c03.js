  ...
  lessons: [
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strncmp", code: "#include <unistd.h>\n\n// --- Helpers pour le main de test ---\nvoid ft_putchar(char c)\n{\n    write(1, &c, 1);          // Ecrit 1 caractere sur stdout\n}\n\nvoid ft_putnbr(int nb)\n{\n    if (nb < 0)                // Si negatif, affiche le signe\n    {\n        ft_putchar('-');\n        nb = -nb;              // Rend le nombre positif\n    }\n    if (nb >= 10)              // S'il reste des chiffres a gauche\n        ft_putnbr(nb / 10);   // Recursion sur les chiffres superieurs\n    ft_putchar(nb % 10 + '0');// Affiche le dernier chiffre en ASCII\n}\n\nint ft_strncmp(char *s1, char *s2, unsigned int n)\n{\n    unsigned int i;            // Index pour parcourir les deux strings\n\n    // Si n == 0, rien a comparer → les strings sont considerees egales\n    if (n == 0)\n        return (0);\n    i = 0;                     // Commence au debut des deux strings\n    // Avance tant que les chars sont egaux, non-nuls, et dans la limite de n\n    while (s1[i] && s2[i] && s1[i] == s2[i] && i < n - 1)\n        i++;                   // Passe au caractere suivant\n    // Retourne la difference au point d'arret (0 si les n premiers sont identiques)\n    return (s1[i] - s2[i]);\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr(ft_strncmp(\"abcX\", \"abcY\", 3)); // Attendu : 0 (compare seulement 3 chars)\n    write(1, \"\\n\", 1);                          // Saut de ligne\n    ft_putnbr(ft_strncmp(\"abc\", \"abd\", 2));     // Attendu : 0 (compare seulement 2 chars)\n    write(1, \"\\n\", 1);\n    return (0);                                  // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strncmp.c -o test && ./test", expectedOutput: "0\\n0", explanation: "Si n == 0 → 0. Sinon, on compare comme strcmp mais on s'arrête à n - 1 (car on compare aussi le char à la position n-1). Si les n premiers chars sont identiques → 0." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strcat", code: "#include <unistd.h>\n\n// --- Helper pour le main de test ---\nvoid ft_putstr(char *str)\n{\n    while (*str)               // Tant qu'on n'a pas atteint le \\0\n        write(1, str++, 1);    // Affiche le char courant et avance le pointeur\n}\n\nchar *ft_strcat(char *dest, char *src)\n{\n    int i;                     // Index pour parcourir dest puis ecrire\n    int j;                     // Index pour parcourir src\n\n    i = 0;                     // Commence au debut de dest\n    // Phase 1 : on avance jusqu'au \\0 de dest\n    while (dest[i])\n        i++;                   // i pointe maintenant sur le \\0 de dest\n    j = 0;                     // Commence au debut de src\n    // Phase 2 : on copie src caractere par caractere a la suite de dest\n    while (src[j])\n    {\n        dest[i] = src[j];      // Copie le char courant de src dans dest\n        i++;                   // Avance dans dest\n        j++;                   // Avance dans src\n    }\n    dest[i] = '\\0';            // Termine la string concatenee avec un \\0\n    return (dest);             // Retourne dest (convention de strcat)\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    char dest[30] = \"Hello \";  // Buffer assez grand pour contenir le resultat\n\n    ft_strcat(dest, \"World!\"); // Colle \"World!\" a la fin de \"Hello \"\n    ft_putstr(dest);           // Affiche \"Hello World!\"\n    write(1, \"\\n\", 1);        // Saut de ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strcat.c -o test && ./test", expectedOutput: "Hello World!", explanation: "On va au \\0 de dest, on copie src char par char, on termine avec un nouveau \\0. Le \\0 original de dest est écrasé par le premier char de src." },
          ...
          { type: "reveal", label: "Voir la solution ft_strncat", code: "#include <unistd.h>\n\n// --- Helper pour le main de test ---\nvoid ft_putstr(char *str)\n{\n    while (*str)               // Tant qu'on n'a pas atteint le \\0\n        write(1, str++, 1);    // Affiche le char courant et avance le pointeur\n}\n\nchar *ft_strncat(char *dest, char *src, unsigned int nb)\n{\n    unsigned int i;            // Index pour parcourir dest puis ecrire\n    unsigned int j;            // Index pour parcourir src\n\n    i = 0;                     // Commence au debut de dest\n    // Phase 1 : on avance jusqu'au \\0 de dest\n    while (dest[i])\n        i++;                   // i pointe maintenant sur le \\0 de dest\n    j = 0;                     // Commence au debut de src\n    // Phase 2 : copie src mais au maximum nb caracteres\n    while (src[j] && j < nb)\n    {\n        dest[i] = src[j];      // Copie le char courant de src dans dest\n        i++;                   // Avance dans dest\n        j++;                   // Avance dans src\n    }\n    // strncat met TOUJOURS un \\0 (contrairement a strncpy)\n    dest[i] = '\\0';            // Termine la string concatenee\n    return (dest);             // Retourne dest (convention de strncat)\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    char dest[30] = \"Hello \";  // Buffer assez grand pour le resultat\n\n    ft_strncat(dest, \"World!!!\", 6); // Copie seulement 6 chars de src (\"World!\")\n    ft_putstr(dest);           // Affiche \"Hello World!\"\n    write(1, \"\\n\", 1);        // Saut de ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strncat.c -o test && ./test", expectedOutput: "Hello World!", explanation: "Comme ft_strcat mais on copie au max nb chars de src. Le \\0 est TOUJOURS ajouté (contrairement à strncpy)." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strstr", code: "#include <unistd.h>\n\n// --- Helper pour le main de test ---\nvoid ft_putstr(char *str)\n{\n    while (*str)               // Tant qu'on n'a pas atteint le \\0\n        write(1, str++, 1);    // Affiche le char courant et avance le pointeur\n}\n\nchar *ft_strstr(char *str, char *to_find)\n{\n    int i;                     // Index dans str (position de depart de chaque tentative)\n    int j;                     // Index dans to_find (progression du match)\n\n    // Si to_find est vide, retourne str par convention (comportement de strstr)\n    if (to_find[0] == '\\0')\n        return (str);\n    i = 0;                     // Commence au debut de str\n    // Boucle externe : teste chaque position de str comme debut de match\n    while (str[i])\n    {\n        j = 0;                 // Reinitialise l'index de to_find a chaque position\n        // Boucle interne : compare str[i+j] avec to_find[j] tant qu'ils matchent\n        while (str[i + j] && to_find[j]\n               && str[i + j] == to_find[j])\n            j++;               // Les chars matchent, avance dans to_find\n        // Si on a parcouru tout to_find, c'est qu'on a un match complet\n        if (to_find[j] == '\\0')\n            return (&str[i]);  // Retourne un pointeur vers le debut du match dans str\n        i++;                   // Pas de match ici, essaie la position suivante\n    }\n    return (0);                // Aucun match trouve, retourne NULL\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    char *result;              // Pointeur pour stocker le resultat de strstr\n\n    result = ft_strstr(\"Hello World\", \"World\"); // Cherche \"World\" dans \"Hello World\"\n    if (result)                // Si le match a ete trouve\n        ft_putstr(result);     // Affiche depuis le debut du match : \"World\"\n    write(1, \"\\n\", 1);        // Saut de ligne\n    result = ft_strstr(\"Hello World\", \"xyz\");   // Cherche \"xyz\" (absent de str)\n    if (result)                // Si trouve (ne sera pas le cas ici)\n        ft_putstr(result);\n    else\n        ft_putstr(\"(null)\");   // Affiche \"(null)\" car non trouve\n    write(1, \"\\n\", 1);        // Saut de ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strstr.c -o test && ./test", expectedOutput: "World\\n(null)", explanation: "Pour chaque position i dans str, on vérifie si to_find commence là. L'astuce str[i + j] permet de comparer sans modifier i. Si on parcourt tout to_find (to_find[j] == '\\0'), c'est trouvé." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strlcat", code: "#include <unistd.h>\n\n// --- Helpers pour le main de test ---\nvoid ft_putchar(char c)\n{\n    write(1, &c, 1);          // Ecrit 1 caractere sur stdout\n}\n\nvoid ft_putnbr(int nb)\n{\n    if (nb < 0)                // Si negatif, affiche le signe\n    {\n        ft_putchar('-');\n        nb = -nb;              // Rend le nombre positif\n    }\n    if (nb >= 10)              // S'il reste des chiffres a gauche\n        ft_putnbr(nb / 10);   // Recursion sur les chiffres superieurs\n    ft_putchar(nb % 10 + '0');// Affiche le dernier chiffre en ASCII\n}\n\nvoid ft_putstr(char *str)\n{\n    while (*str)               // Tant qu'on n'a pas atteint le \\0\n        write(1, str++, 1);    // Affiche le char courant et avance le pointeur\n}\n\nunsigned int ft_strlcat(char *dest, char *src, unsigned int size)\n{\n    unsigned int d_len;        // Longueur de dest (limitee a size)\n    unsigned int s_len;        // Longueur de src\n    unsigned int i;            // Index pour copier src\n\n    d_len = 0;                 // Initialise le compteur de longueur de dest\n    // Mesure la longueur de dest, sans depasser size (securite buffer)\n    while (dest[d_len] && d_len < size)\n        d_len++;               // Compte les chars de dest dans la limite de size\n    s_len = 0;                 // Initialise le compteur de longueur de src\n    // Mesure la longueur complete de src (pour le calcul du retour)\n    while (src[s_len])\n        s_len++;               // Compte tous les chars de src\n    // Si dest remplit deja le buffer, retour special : size + s_len\n    if (d_len >= size)\n        return (size + s_len);\n    i = 0;                     // Initialise l'index de copie\n    // Copie src a la suite de dest, en respectant la limite du buffer\n    while (src[i] && d_len + i < size - 1)\n    {\n        dest[d_len + i] = src[i]; // Ecrit le char de src dans dest\n        i++;                   // Avance dans src\n    }\n    dest[d_len + i] = '\\0';   // Termine toujours par un \\0\n    // Retourne la longueur theorique (d_len + s_len), sans troncation\n    return (d_len + s_len);\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    char dest[20] = \"Hello \";  // Buffer de taille 20\n    unsigned int ret;           // Stocke la valeur de retour\n\n    ret = ft_strlcat(dest, \"World!\", 20); // Concatene dans un buffer de taille 20\n    ft_putstr(dest);           // Affiche \"Hello World!\"\n    write(1, \" \", 1);         // Espace avant le retour\n    ft_putnbr(ret);            // Affiche 12 (6 + 6 = longueur theorique)\n    write(1, \"\\n\", 1);        // Saut de ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strlcat.c -o test && ./test", expectedOutput: "Hello World! 12", explanation: "On mesure dest (sans dépasser size), on mesure src. Si dest remplit déjà le buffer → retour spécial. Sinon on concatène avec limite et on retourne la longueur théorique." },
    ...
    },
    {
      ...
      content: {
        ...
        quiz: {
          type: "truefalse",
          question: "strncat ajoute toujours un \\0 à la fin, contrairement à strncpy qui ne le fait que si src est plus courte que n.",
          options: ["Vrai", "Faux"],
          correct: 0,
