    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_div_mod", code: "#include <unistd.h>\n\nvoid\tft_putchar(char c)\n{\n\twrite(1, &c, 1);\n}\n\nvoid\tft_putnbr(int nb)\n{\n\tif (nb < 0)\n\t{\n\t\tft_putchar('-');\n\t\tnb = -nb;\n\t}\n\tif (nb >= 10)\n\t\tft_putnbr(nb / 10);\n\tft_putchar(nb % 10 + '0');\n}\n\nvoid ft_div_mod(int a, int b, int *div, int *mod) // a et b = valeurs, div et mod = pointeurs pour stocker les resultats\n{\n    *div = a / b;              // Stocke le quotient entier a l'adresse pointee par div\n    *mod = a % b;              // Stocke le reste de la division a l'adresse pointee par mod\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    int div;                   // Contiendra le quotient\n    int mod;                   // Contiendra le reste\n\n    ft_div_mod(10, 3, &div, &mod); // Calcule 10/3 et 10%3, stocke via pointeurs\n    ft_putnbr(div);            // Affiche 3 (quotient de 10/3)\n    write(1, \" \", 1);\n    ft_putnbr(mod);            // Affiche 1 (reste de 10/3)\n    write(1, \"\\n\", 1);\n    return (0);\n}", explanation: "*div = a / b stocke le quotient à l'adresse pointée par div. Idem pour mod. L'appelant utilise ft_div_mod(10, 3, &quotient, &reste).", compileCmd: "gcc -Wall -Wextra -Werror ft_div_mod.c -o test && ./test", expectedOutput: "3 1" },
          ...
          { type: "reveal", label: "Voir la solution ft_ultimate_div_mod", code: "#include <unistd.h>\n\nvoid\tft_putchar(char c)\n{\n\twrite(1, &c, 1);\n}\n\nvoid\tft_putnbr(int nb)\n{\n\tif (nb < 0)\n\t{\n\t\tft_putchar('-');\n\t\tnb = -nb;\n\t}\n\tif (nb >= 10)\n\t\tft_putnbr(nb / 10);\n\tft_putchar(nb % 10 + '0');\n}\n\nvoid ft_ultimate_div_mod(int *a, int *b) // a et b sont des pointeurs, ils servent d'entree ET de sortie\n{\n    int tmp_a;                 // Sauvegarde de *a avant de l'ecraser\n\n    tmp_a = *a;                // Sauvegarde la valeur de a (sinon on la perd)\n    *a = tmp_a / *b;           // Ecrase a avec le quotient\n    *b = tmp_a % *b;           // Ecrase b avec le reste (utilise tmp_a, pas *a qui est deja modifie)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    int a;                     // Contiendra d'abord le dividende, puis le quotient\n    int b;                     // Contiendra d'abord le diviseur, puis le reste\n\n    a = 10;                    // Dividende\n    b = 3;                     // Diviseur\n    ft_ultimate_div_mod(&a, &b); // Apres l'appel, a = quotient, b = reste\n    ft_putnbr(a);              // Affiche 3 (quotient)\n    write(1, \" \", 1);\n    ft_putnbr(b);              // Affiche 1 (reste)\n    write(1, \"\\n\", 1);\n    return (0);\n}", explanation: "Si on écrit *a = *a / *b sans sauvegarder, le calcul de *b utilisera le mauvais *a (déjà écrasé). La variable temporaire est indispensable.", compileCmd: "gcc -Wall -Wextra -Werror ft_ultimate_div_mod.c -o test && ./test", expectedOutput: "3 1" },
