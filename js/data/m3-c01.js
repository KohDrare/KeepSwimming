    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_ft", code: "void ft_ft(int *nbr)\n{\n    *nbr = 42;  // Va à l'adresse, y met 42\n}", explanation: "*nbr = 42 accède au tiroir dont l'adresse est dans nbr et y place 42. C'est la base du passage par adresse." },
          ...
          { type: "reveal", label: "Voir la solution ft_swap", code: "void ft_swap(int *a, int *b)\n{\n    int tmp;\n\n    tmp = *a;   // Sauvegarde la valeur de a\n    *a = *b;    // Met la valeur de b dans a\n    *b = tmp;   // Met l'ancienne valeur de a dans b\n}", explanation: "Le swap en 3 étapes avec une variable temporaire. L'appel se fait avec ft_swap(&x, &y) pour passer les adresses." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_div_mod", code: "void ft_div_mod(int a, int b, int *div, int *mod)\n{\n    *div = a / b;   // Stocke le quotient\n    *mod = a % b;   // Stocke le reste\n}", explanation: "*div = a / b stocke le quotient à l'adresse pointée par div. Idem pour mod. L'appelant utilise ft_div_mod(10, 3, &quotient, &reste)." },
          ...
          { type: "reveal", label: "Voir la solution ft_ultimate_div_mod", code: "void ft_ultimate_div_mod(int *a, int *b)\n{\n    int tmp_a;\n\n    tmp_a = *a;       // Sauvegarde la valeur de a\n    *a = tmp_a / *b;  // a reçoit le quotient\n    *b = tmp_a % *b;  // b reçoit le reste\n}", explanation: "Si on écrit *a = *a / *b sans sauvegarder, le calcul de *b utilisera le mauvais *a (déjà écrasé). La variable temporaire est indispensable." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strlen", code: "int ft_strlen(char *str)\n{\n    int i;\n\n    i = 0;\n    while (str[i])  // '\\0' == 0 == FAUX\n        i++;\n    return (i);\n}", explanation: "On parcourt la string caractère par caractère. Quand on tombe sur '\\0', la boucle s'arrête et i contient la longueur." },
          ...
          { type: "reveal", label: "Voir la solution ft_putstr", code: "void ft_putstr(char *str)\n{\n    int i;\n\n    i = 0;\n    while (str[i] != '\\0')\n    {\n        write(1, &str[i], 1);\n        i++;\n    }\n}", explanation: "On parcourt la string et on affiche chaque caractère avec write. La boucle s'arrête au '\\0'." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution", code: "void ft_rev_int_tab(int *tab, int size)\n{\n    int i;\n    int tmp;\n\n    i = 0;\n    while (i < size / 2)\n    {\n        tmp = tab[i];\n        tab[i] = tab[size - 1 - i];\n        tab[size - 1 - i] = tmp;\n        i++;\n    }\n}", explanation: "On échange les éléments symétriques : tab[0] ↔ tab[4], tab[1] ↔ tab[3], etc. La boucle s'arrête au milieu car au-delà on re-échangerait ce qu'on a déjà fait." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution", code: "void ft_sort_int_tab(int *tab, int size)\n{\n    int i;\n    int tmp;\n    int sorted;\n\n    sorted = 0;\n    while (!sorted)\n    {\n        sorted = 1;\n        i = 0;\n        while (i < size - 1)\n        {\n            if (tab[i] > tab[i + 1])\n            {\n                tmp = tab[i];\n                tab[i] = tab[i + 1];\n                tab[i + 1] = tmp;\n                sorted = 0;\n            }\n            i++;\n        }\n    }\n}", explanation: "Le tri à bulles : on compare chaque paire de voisins et on échange si l'ordre est mauvais. On recommence tant qu'on a fait au moins un échange. Simple à coder, suffisant pour la Piscine." },
