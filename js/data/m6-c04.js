    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_atoi", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)\n{\n    write(1, &c, 1);\n}\n\nvoid ft_putnbr(int nb)\n{\n    if (nb < 0)\n    {\n        ft_putchar('-');\n        nb = -nb;\n    }\n    if (nb >= 10)\n        ft_putnbr(nb / 10);\n    ft_putchar(nb % 10 + '0');\n}\n\nint ft_atoi(char *str)\n{\n    int i;\n    int sign;\n    int result;\n\n    i = 0;\n    sign = 1;\n    result = 0;\n    // Phase 1 : on saute les whitespace (espace, tab, newline, etc.)\n    while (str[i] == ' ' || (str[i] >= 9 && str[i] <= 13))\n        i++;\n    // Phase 2 : on gere les signes multiples (particularite C04)\n    while (str[i] == '+' || str[i] == '-')\n    {\n        if (str[i] == '-')\n            sign *= -1; // Chaque '-' inverse le signe\n        i++;\n    }\n    // Phase 3 : on construit le nombre chiffre par chiffre\n    while (str[i] >= '0' && str[i] <= '9')\n    {\n        result = result * 10 + (str[i] - '0'); // Decale et ajoute le chiffre\n        i++;\n    }\n    return (result * sign); // Applique le signe final\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr(ft_atoi(\"  42\")); // Attendu : 42\n    write(1, \"\\n\", 1);\n    ft_putnbr(ft_atoi(\"  ---+--+1234ab567\")); // Attendu : -1234 (5 '-' = negatif)\n    write(1, \"\\n\", 1);\n    ft_putnbr(ft_atoi(\"0\")); // Attendu : 0\n    write(1, \"\\n\", 1);\n    return (0);\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_atoi.c -o test && ./test", expectedOutput: "42\\n-1234\\n0", explanation: "3 boucles : skip whitespace, compte les signes (impair de '-' = négatif), construit le nombre. S'arrête au premier non-chiffre." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_putnbr_base", code: "#include <unistd.h>\n\nint check_base(char *base)\n{\n    int i;\n    int j;\n\n    i = 0;\n    while (base[i])\n    {\n        if (base[i] == '+' || base[i] == '-')\n            return (0);\n        j = i + 1;\n        while (base[j])\n        {\n            if (base[i] == base[j])\n                return (0);\n            j++;\n        }\n        i++;\n    }\n    return (i >= 2);\n}\n\nvoid ft_putnbr_base(int nbr, char *base)\n{\n    int base_len;\n    long n;\n\n    // Verifie que la base est valide (>= 2 chars, pas de doublons, pas de +/-)\n    if (!check_base(base))\n        return ;\n    base_len = 0;\n    // Calcule la longueur de la base (= le nombre de symboles)\n    while (base[base_len])\n        base_len++;\n    // Utilise un long pour gerer INT_MIN sans overflow\n    n = nbr;\n    if (n < 0)\n    {\n        write(1, \"-\", 1); // Affiche le signe negatif\n        n = -n;\n    }\n    // Recursion : traite d'abord les chiffres de gauche\n    if (n >= base_len)\n        ft_putnbr_base(n / base_len, base);\n    // Affiche le symbole correspondant au reste (dernier chiffre)\n    write(1, &base[n % base_len], 1);\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr_base(42, \"0123456789\"); // Attendu : \"42\" (base 10)\n    write(1, \"\\n\", 1);\n    ft_putnbr_base(42, \"0123456789abcdef\"); // Attendu : \"2a\" (base 16)\n    write(1, \"\\n\", 1);\n    ft_putnbr_base(42, \"01\"); // Attendu : \"101010\" (base 2)\n    write(1, \"\\n\", 1);\n    ft_putnbr_base(-42, \"0123456789\"); // Attendu : \"-42\" (negatif)\n    write(1, \"\\n\", 1);\n    return (0);\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_putnbr_base.c -o test && ./test", expectedOutput: "42\\n2a\\n101010\\n-42", explanation: "On vérifie la base, puis c'est ft_putnbr avec base[n % base_len] au lieu de n % 10 + '0'. Le long gère INT_MIN." },
