    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_putnbr", code: "#include <unistd.h>\n\nvoid ft_putnbr(int nb)\n{\n    char c;                    // Variable pour stocker le chiffre a afficher\n\n    // Cas special : INT_MIN ne peut pas etre inverse (-nb overflow)\n    if (nb == -2147483648)\n    {\n        write(1, \"-2147483648\", 11); // Affiche en dur (seule solution safe)\n        return ;               // Quitte la fonction, pas besoin de continuer\n    }\n    // Si negatif, affiche le signe '-' et inverse le nombre\n    if (nb < 0)\n    {\n        write(1, \"-\", 1);     // Affiche le signe moins\n        nb = -nb;              // Rend le nombre positif pour le traitement\n    }\n    // Recursion : traite d'abord les chiffres de gauche (dizaines, centaines...)\n    if (nb >= 10)\n        ft_putnbr(nb / 10);   // Appel recursif avec le nombre sans son dernier chiffre\n    // Affiche le dernier chiffre (nb % 10 donne 0-9, + '0' convertit en ASCII)\n    c = nb % 10 + '0';        // Convertit le chiffre en caractere ASCII\n    write(1, &c, 1);          // Ecrit ce caractere sur stdout\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr(42);             // Attendu : \"42\"\n    write(1, \"\\n\", 1);        // Saut de ligne entre chaque test\n    ft_putnbr(-42);            // Attendu : \"-42\" (signe + nombre)\n    write(1, \"\\n\", 1);\n    ft_putnbr(0);              // Attendu : \"0\" (cas simple)\n    write(1, \"\\n\", 1);\n    ft_putnbr(-2147483648);    // Attendu : \"-2147483648\" (INT_MIN, cas special)\n    write(1, \"\\n\", 1);\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_putnbr.c -o test && ./test", expectedOutput: "42\\n-42\\n0\\n-2147483648", explanation: "INT_MIN est géré en dur (hardcodé). Pour le reste : on gère le signe, puis la récursion traite les chiffres de gauche à droite grâce à l'empilement des appels." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_atoi", code: "#include <unistd.h>\n\n// --- Helpers pour le main de test ---\nvoid ft_putchar(char c)\n{\n    write(1, &c, 1);          // Ecrit 1 caractere sur stdout\n}\n\nvoid ft_putnbr(int nb)\n{\n    if (nb < 0)                // Si negatif, affiche le signe\n    {\n        ft_putchar('-');\n        nb = -nb;              // Rend le nombre positif\n    }\n    if (nb >= 10)              // S'il reste des chiffres a gauche\n        ft_putnbr(nb / 10);   // Recursion sur les chiffres superieurs\n    ft_putchar(nb % 10 + '0');// Affiche le dernier chiffre en ASCII\n}\n\nint ft_atoi(char *str)\n{\n    int i;                     // Index pour parcourir la string\n    int sign;                  // Signe final : 1 (positif) ou -1 (negatif)\n    int result;                // Nombre en cours de construction\n\n    i = 0;                     // Commence au debut de la string\n    sign = 1;                  // Par defaut le nombre est positif\n    result = 0;                // Le resultat commence a 0\n    // Phase 1 : saute les whitespace (espace, tab, newline, etc.)\n    while (str[i] == ' ' || (str[i] >= 9 && str[i] <= 13))\n        i++;                   // Ignore chaque whitespace\n    // Phase 2 : gere les signes multiples (particularite du sujet C04)\n    while (str[i] == '+' || str[i] == '-')\n    {\n        if (str[i] == '-')\n            sign *= -1;        // Chaque '-' inverse le signe (pair = +, impair = -)\n        i++;                   // Passe au caractere suivant\n    }\n    // Phase 3 : construit le nombre chiffre par chiffre\n    while (str[i] >= '0' && str[i] <= '9')\n    {\n        result = result * 10 + (str[i] - '0'); // Decale d'un rang et ajoute le chiffre\n        i++;                   // Passe au chiffre suivant\n    }\n    return (result * sign);    // Applique le signe et retourne le resultat\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr(ft_atoi(\"  42\"));              // Attendu : 42 (whitespace ignores)\n    write(1, \"\\n\", 1);                        // Saut de ligne\n    ft_putnbr(ft_atoi(\"  ---+--+1234ab567\")); // Attendu : -1234 (5 '-' = negatif, stop a 'a')\n    write(1, \"\\n\", 1);\n    ft_putnbr(ft_atoi(\"0\"));                  // Attendu : 0 (cas simple)\n    write(1, \"\\n\", 1);\n    return (0);                                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_atoi.c -o test && ./test", expectedOutput: "42\\n-1234\\n0", explanation: "3 boucles : skip whitespace, compte les signes (impair de '-' = négatif), construit le nombre. S'arrête au premier non-chiffre." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_putnbr_base", code: "#include <unistd.h>\n\n// Verifie que la base est valide : >= 2 chars, pas de +/-, pas de doublons\nint check_base(char *base)\n{\n    int i;                     // Index pour le char courant\n    int j;                     // Index pour chercher les doublons\n\n    i = 0;                     // Commence au debut de la base\n    while (base[i])            // Parcourt chaque caractere de la base\n    {\n        if (base[i] == '+' || base[i] == '-') // +/- sont interdits (reserves aux signes)\n            return (0);        // Base invalide\n        j = i + 1;             // Compare avec tous les chars suivants\n        while (base[j])        // Parcourt le reste de la base\n        {\n            if (base[i] == base[j]) // Si doublon trouve\n                return (0);    // Base invalide\n            j++;               // Passe au char suivant\n        }\n        i++;                   // Passe au char suivant a verifier\n    }\n    return (i >= 2);           // Valide seulement si au moins 2 symboles\n}\n\nvoid ft_putnbr_base(int nbr, char *base)\n{\n    int base_len;              // Nombre de symboles dans la base\n    long n;                    // Version long de nbr (pour gerer INT_MIN)\n\n    // Verifie que la base est valide avant de continuer\n    if (!check_base(base))\n        return ;               // Base invalide, on ne fait rien\n    base_len = 0;              // Initialise le compteur de longueur\n    // Calcule la longueur de la base (= le nombre de symboles)\n    while (base[base_len])\n        base_len++;            // Compte chaque symbole\n    // Utilise un long pour eviter l'overflow avec INT_MIN (-2147483648)\n    n = nbr;                   // Copie dans un long\n    if (n < 0)                 // Si le nombre est negatif\n    {\n        write(1, \"-\", 1);     // Affiche le signe moins\n        n = -n;                // Rend le nombre positif (safe grace au long)\n    }\n    // Recursion : traite d'abord les chiffres de gauche\n    if (n >= base_len)\n        ft_putnbr_base(n / base_len, base); // Appel recursif sans le dernier chiffre\n    // Affiche le symbole correspondant au dernier chiffre (reste de la division)\n    write(1, &base[n % base_len], 1); // base[reste] donne le bon symbole\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr_base(42, \"0123456789\");      // Attendu : \"42\" (base 10, decimal)\n    write(1, \"\\n\", 1);                      // Saut de ligne\n    ft_putnbr_base(42, \"0123456789abcdef\"); // Attendu : \"2a\" (base 16, hexadecimal)\n    write(1, \"\\n\", 1);\n    ft_putnbr_base(42, \"01\");               // Attendu : \"101010\" (base 2, binaire)\n    write(1, \"\\n\", 1);\n    ft_putnbr_base(-42, \"0123456789\");      // Attendu : \"-42\" (nombre negatif)\n    write(1, \"\\n\", 1);\n    return (0);                              // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_putnbr_base.c -o test && ./test", expectedOutput: "42\\n2a\\n101010\\n-42", explanation: "On vérifie la base, puis c'est ft_putnbr avec base[n % base_len] au lieu de n % 10 + '0'. Le long gère INT_MIN." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_atoi_base", code: "#include <unistd.h>\n\n// Verifie que la base est valide : >= 2 chars, pas de +/-, pas de doublons\nint check_base(char *base)\n{\n    int i;                     // Index pour le char courant\n    int j;                     // Index pour chercher les doublons\n\n    i = 0;                     // Commence au debut de la base\n    while (base[i])            // Parcourt chaque caractere de la base\n    {\n        if (base[i] == '+' || base[i] == '-') // +/- reserves aux signes\n            return (0);        // Base invalide\n        j = i + 1;             // Compare avec tous les chars suivants\n        while (base[j])        // Parcourt le reste pour detecter les doublons\n        {\n            if (base[i] == base[j]) // Doublon trouve\n                return (0);    // Base invalide\n            j++;               // Passe au char suivant\n        }\n        i++;                   // Passe au char suivant a verifier\n    }\n    return (i >= 2);           // Valide seulement si au moins 2 symboles\n}\n\n// Cherche le caractere c dans la base et retourne son index (= sa valeur)\nint get_index(char c, char *base)\n{\n    int i;                     // Index pour parcourir la base\n\n    i = 0;                     // Commence au debut de la base\n    while (base[i])            // Parcourt chaque symbole de la base\n    {\n        if (base[i] == c)      // Si le symbole correspond au char cherche\n            return (i);        // Retourne l'index (= la valeur numerique)\n        i++;                   // Passe au symbole suivant\n    }\n    return (-1);               // Char absent de la base\n}\n\nint ft_atoi_base(char *str, char *base)\n{\n    int i;                     // Index pour parcourir la string\n    int sign;                  // Signe final : 1 ou -1\n    int result;                // Nombre en cours de construction\n    int base_len;              // Nombre de symboles dans la base\n    int idx;                   // Index du char courant dans la base (= sa valeur)\n\n    // Verifie la base avant de commencer\n    if (!check_base(base))\n        return (0);            // Base invalide, retourne 0\n    base_len = 0;              // Initialise le compteur de longueur\n    // Calcule la longueur de la base\n    while (base[base_len])\n        base_len++;            // Compte chaque symbole\n    i = 0;                     // Commence au debut de la string\n    sign = 1;                  // Par defaut le nombre est positif\n    result = 0;                // Le resultat commence a 0\n    // Phase 1 : saute les whitespace (espace, tab, newline, etc.)\n    while (str[i] == ' ' || (str[i] >= 9 && str[i] <= 13))\n        i++;                   // Ignore chaque whitespace\n    // Phase 2 : gere les signes multiples (comme ft_atoi C04)\n    while (str[i] == '+' || str[i] == '-')\n    {\n        if (str[i] == '-')\n            sign *= -1;        // Chaque '-' inverse le signe\n        i++;                   // Passe au caractere suivant\n    }\n    // Phase 3 : construit le nombre avec base_len au lieu de 10\n    while ((idx = get_index(str[i], base)) != -1)\n    {\n        result = result * base_len + idx; // Decale et ajoute la valeur du symbole\n        i++;                   // Passe au symbole suivant\n    }\n    return (result * sign);    // Applique le signe et retourne le resultat\n}\n\n// --- Helpers pour le main de test ---\nvoid ft_putchar(char c)\n{\n    write(1, &c, 1);          // Ecrit 1 caractere sur stdout\n}\n\nvoid ft_putnbr(int nb)\n{\n    if (nb < 0)                // Si negatif, affiche le signe\n    {\n        ft_putchar('-');\n        nb = -nb;              // Rend le nombre positif\n    }\n    if (nb >= 10)              // S'il reste des chiffres a gauche\n        ft_putnbr(nb / 10);   // Recursion sur les chiffres superieurs\n    ft_putchar(nb % 10 + '0');// Affiche le dernier chiffre en ASCII\n}\n\n// --- Main de test (a retirer avant de rendre) ---\nint main(void)\n{\n    ft_putnbr(ft_atoi_base(\"2a\", \"0123456789abcdef\")); // Attendu : 42 (hexa → decimal)\n    write(1, \"\\n\", 1);                                  // Saut de ligne\n    ft_putnbr(ft_atoi_base(\"101010\", \"01\"));             // Attendu : 42 (binaire → decimal)\n    write(1, \"\\n\", 1);\n    ft_putnbr(ft_atoi_base(\"-42\", \"0123456789\"));        // Attendu : -42 (decimal negatif)\n    write(1, \"\\n\", 1);\n    return (0);                                          // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_atoi_base.c -o test && ./test", expectedOutput: "42\\n42\\n-42", explanation: "get_index trouve la valeur d'un symbole dans la base. Le reste est ft_atoi avec base_len au lieu de 10 et get_index au lieu de str[i] - '0'." },
