  ...
  lessons: [
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_strlen", code: "int ft_strlen(char *str)\n{\n    int i;\n\n    i = 0;\n    while (str[i])\n        i++;\n    return (i);\n}", explanation: "On parcourt jusqu'au \\0 et on retourne le compteur. C'est la base de presque tout en C." },
          ...
          { type: "reveal", label: "Voir la solution ft_putstr", code: "void ft_putstr(char *str)\n{\n    int i;\n\n    i = 0;\n    while (str[i])\n    {\n        write(1, &str[i], 1);\n        i++;\n    }\n}", explanation: "On parcourt la string et on affiche chaque caractère. Variante plus courte : write(1, str, ft_strlen(str))." },
        ...
        quiz: {
          type: "truefalse",
          statement: "ft_strlen(\"Hello\") retourne 6 car elle compte le \\0.",
          correct: false,
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_putnbr", code: "void ft_putnbr(int nb)\n{\n    char c;\n\n    if (nb == -2147483648)\n    {\n        write(1, \"-2147483648\", 11);\n        return ;\n    }\n    if (nb < 0)\n    {\n        write(1, \"-\", 1);\n        nb = -nb;\n    }\n    if (nb >= 10)\n        ft_putnbr(nb / 10);\n    c = nb % 10 + '0';\n    write(1, &c, 1);\n}", explanation: "INT_MIN est géré en dur (hardcodé). Pour le reste : on gère le signe, puis la récursion traite les chiffres de gauche à droite grâce à l'empilement des appels." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_atoi", code: "int ft_atoi(char *str)\n{\n    int i;\n    int sign;\n    int result;\n\n    i = 0;\n    sign = 1;\n    result = 0;\n    while (str[i] == ' ' || (str[i] >= 9 && str[i] <= 13))\n        i++;\n    while (str[i] == '+' || str[i] == '-')\n    {\n        if (str[i] == '-')\n            sign *= -1;\n        i++;\n    }\n    while (str[i] >= '0' && str[i] <= '9')\n    {\n        result = result * 10 + (str[i] - '0');\n        i++;\n    }\n    return (result * sign);\n}", explanation: "3 boucles : skip whitespace, compte les signes (impair de '-' = négatif), construit le nombre. S'arrête au premier non-chiffre." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_putnbr_base", code: "int check_base(char *base)\n{\n    int i;\n    int j;\n\n    i = 0;\n    while (base[i])\n    {\n        if (base[i] == '+' || base[i] == '-')\n            return (0);\n        j = i + 1;\n        while (base[j])\n        {\n            if (base[i] == base[j])\n                return (0);\n            j++;\n        }\n        i++;\n    }\n    return (i >= 2);\n}\n\nvoid ft_putnbr_base(int nbr, char *base)\n{\n    int base_len;\n    long n;\n\n    if (!check_base(base))\n        return ;\n    base_len = 0;\n    while (base[base_len])\n        base_len++;\n    n = nbr;\n    if (n < 0)\n    {\n        write(1, \"-\", 1);\n        n = -n;\n    }\n    if (n >= base_len)\n        ft_putnbr_base(n / base_len, base);\n    write(1, &base[n % base_len], 1);\n}", explanation: "On vérifie la base, puis c'est ft_putnbr avec base[n % base_len] au lieu de n % 10 + '0'. Le long gère INT_MIN." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "reveal", label: "Voir la solution ft_atoi_base", code: "int get_index(char c, char *base)\n{\n    int i;\n\n    i = 0;\n    while (base[i])\n    {\n        if (base[i] == c)\n            return (i);\n        i++;\n    }\n    return (-1);\n}\n\nint ft_atoi_base(char *str, char *base)\n{\n    int i;\n    int sign;\n    int result;\n    int base_len;\n    int idx;\n\n    if (!check_base(base))\n        return (0);\n    base_len = 0;\n    while (base[base_len])\n        base_len++;\n    i = 0;\n    sign = 1;\n    result = 0;\n    while (str[i] == ' ' || (str[i] >= 9 && str[i] <= 13))\n        i++;\n    while (str[i] == '+' || str[i] == '-')\n    {\n        if (str[i] == '-')\n            sign *= -1;\n        i++;\n    }\n    while ((idx = get_index(str[i], base)) != -1)\n    {\n        result = result * base_len + idx;\n        i++;\n    }\n    return (result * sign);\n}", explanation: "get_index trouve la valeur d'un symbole dans la base. Le reste est ft_atoi avec base_len au lieu de 10 et get_index au lieu de str[i] - '0'." },
