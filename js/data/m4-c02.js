      ...
      content: {
        blocks: [
          ...
          { type: "challenge", questions: [
            "Quel pattern ? Parcourir src et copier chaque caractere dans dest.",
            "Quand s'arreter ? Quand src[i] == \\0.",
            "Que ne faut-il surtout pas oublier ? Copier le \\0 a la fin de dest.",
          ]},
          { type: "reveal", label: "Voir la solution ft_strcpy", code: "#include <unistd.h>\n\nvoid ft_putstr(char *str)      // Affiche une string caractere par caractere\n{\n    while (*str)               // Tant que le caractere n'est pas '\\0'\n        write(1, str++, 1);    // Affiche le char puis avance le pointeur\n}\n\nchar *ft_strcpy(char *dest, char *src) // Copie src dans dest, retourne dest\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (src[i])             // Tant qu'on n'a pas atteint le '\\0' de src\n    {\n        dest[i] = src[i];      // Copie le caractere courant\n        i++;                   // Passe au caractere suivant\n    }\n    dest[i] = '\\0';            // Copie le '\\0' final (indispensable !)\n    return (dest);             // Retourne dest (convention de strcpy)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    char dest[20];             // Buffer de destination assez grand\n\n    ft_strcpy(dest, \"Hello\");  // Copie \"Hello\" dans dest\n    ft_putstr(dest);           // Affiche le contenu de dest\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strcpy.c -o test && ./test", expectedOutput: "Hello", explanation: "On copie caractère par caractère jusqu'au \\0, puis on ajoute le \\0 dans dest. Le return (dest) est la convention de strcpy." },
          ...
          { type: "challenge", questions: [
            "Quelle difference avec strcpy ? On copie au MAXIMUM n caracteres.",
            "Que faire si src est plus courte que n ? Remplir le reste de dest avec des \\0.",
            "Que faire si src est plus longue que n ? On coupe a n, SANS ajouter de \\0.",
          ]},
          { type: "reveal", label: "Voir la solution ft_strncpy", code: "#include <unistd.h>\n\nvoid ft_putstr(char *str)      // Affiche une string caractere par caractere\n{\n    while (*str)               // Tant que le caractere n'est pas '\\0'\n        write(1, str++, 1);    // Affiche le char puis avance le pointeur\n}\n\nchar *ft_strncpy(char *dest, char *src, unsigned int n) // Copie au max n chars de src dans dest\n{\n    unsigned int i;\n\n    i = 0;                     // Index de parcours\n    while (src[i] && i < n)    // Copie tant que src n'est pas finie ET i < n\n    {\n        dest[i] = src[i];      // Copie le caractere courant\n        i++;                   // Passe au suivant\n    }\n    while (i < n)              // Si src etait plus courte que n\n    {\n        dest[i] = '\\0';        // Remplit le reste avec des '\\0'\n        i++;                   // Avance dans dest\n    }\n    return (dest);             // Retourne dest\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    char dest[20];             // Buffer de destination\n\n    ft_strncpy(dest, \"Hello World\", 5); // Copie seulement 5 chars\n    dest[5] = '\\0';            // Ajoute '\\0' manuellement (strncpy ne le fait pas si src >= n)\n    ft_putstr(dest);           // Affiche \"Hello\"\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strncpy.c -o test && ./test", expectedOutput: "Hello", explanation: "Si src est plus courte que n : on remplit de \\0. Si src >= n : on coupe à n, SANS \\0. C'est le comportement standard de strncpy." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "challenge", questions: [
            "Quel pattern ? Parcourir, tester chaque char, return 0 au premier echec.",
            "Comment tester si c'est une lettre ? (str[i] >= 'a' && str[i] <= 'z') ou majuscule.",
            "Que retourner pour une string vide ? 1 (vrai, aucun caractere ne viole la condition).",
          ]},
          { type: "reveal", label: "Voir la solution ft_str_is_alpha", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)        // Affiche un seul caractere\n{\n    write(1, &c, 1);           // Ecrit 1 octet sur stdout\n}\n\nint ft_str_is_alpha(char *str) // Retourne 1 si que des lettres, 0 sinon\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere de la string\n    {\n        if (!((str[i] >= 'a' && str[i] <= 'z')\n            || (str[i] >= 'A' && str[i] <= 'Z'))) // Si ce n'est PAS une lettre\n            return (0);        // Un seul non-alpha suffit : retourne 0\n        i++;                   // Passe au caractere suivant\n    }\n    return (1);                // Tous les chars sont des lettres (ou string vide)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putchar(ft_str_is_alpha(\"Hello\") + '0');   // 1 : que des lettres\n    ft_putchar(ft_str_is_alpha(\"Hello42\") + '0'); // 0 : contient des chiffres\n    ft_putchar(ft_str_is_alpha(\"\") + '0');         // 1 : string vide = vrai\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_str_is_alpha.c -o test && ./test", expectedOutput: "101", explanation: "On parcourt chaque char. Dès qu'un n'est pas une lettre, on retourne 0. Si on finit la boucle, tout est bon → 1. String vide : la boucle ne s'exécute pas → 1." },
          ...
          { type: "challenge", questions: [
            "Quel pattern reutilises-tu ? Le meme que ft_str_is_alpha.",
            "Seul le test change. Lequel ? str[i] >= '0' && str[i] <= '9'.",
          ]},
          { type: "reveal", label: "Voir la solution ft_str_is_numeric", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)        // Affiche un seul caractere\n{\n    write(1, &c, 1);           // Ecrit 1 octet sur stdout\n}\n\nint ft_str_is_numeric(char *str) // Retourne 1 si que des chiffres, 0 sinon\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (!(str[i] >= '0' && str[i] <= '9')) // Si ce n'est PAS un chiffre\n            return (0);        // Retourne 0 immediatement\n        i++;                   // Passe au caractere suivant\n    }\n    return (1);                // Tous les chars sont des chiffres (ou string vide)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putchar(ft_str_is_numeric(\"42\") + '0');    // 1 : que des chiffres\n    ft_putchar(ft_str_is_numeric(\"42abc\") + '0'); // 0 : contient des lettres\n    ft_putchar(ft_str_is_numeric(\"\") + '0');       // 1 : string vide = vrai\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_str_is_numeric.c -o test && ./test", expectedOutput: "101", explanation: "Même pattern : parcourir, tester, retourner 0 au premier caractère invalide. Return 1 si tous sont des chiffres." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "challenge", questions: [
            "Quel test pour les minuscules ? str[i] >= 'a' && str[i] <= 'z'.",
            "Que changes-tu par rapport a ft_str_is_alpha ? On ne garde que la plage minuscule.",
          ]},
          { type: "reveal", label: "Voir la solution ft_str_is_lowercase", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)        // Affiche un seul caractere\n{\n    write(1, &c, 1);           // Ecrit 1 octet sur stdout\n}\n\nint ft_str_is_lowercase(char *str) // Retourne 1 si que des minuscules, 0 sinon\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (!(str[i] >= 'a' && str[i] <= 'z')) // Si ce n'est PAS une minuscule\n            return (0);        // Retourne 0 immediatement\n        i++;                   // Passe au caractere suivant\n    }\n    return (1);                // Tous les chars sont des minuscules (ou string vide)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putchar(ft_str_is_lowercase(\"hello\") + '0');  // 1 : que des minuscules\n    ft_putchar(ft_str_is_lowercase(\"Hello\") + '0');  // 0 : 'H' est majuscule\n    ft_putchar(ft_str_is_lowercase(\"\") + '0');        // 1 : string vide = vrai\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_str_is_lowercase.c -o test && ./test", expectedOutput: "101", explanation: "Seul le test change : on ne vérifie que la plage 'a'-'z'. Le reste du pattern est identique." },
          ...
          { type: "challenge", questions: [
            "Quel test pour les majuscules ? str[i] >= 'A' && str[i] <= 'Z'.",
            "Meme structure que lowercase ? Oui, seules les bornes changent.",
          ]},
          { type: "reveal", label: "Voir la solution ft_str_is_uppercase", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)        // Affiche un seul caractere\n{\n    write(1, &c, 1);           // Ecrit 1 octet sur stdout\n}\n\nint ft_str_is_uppercase(char *str) // Retourne 1 si que des majuscules, 0 sinon\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (!(str[i] >= 'A' && str[i] <= 'Z')) // Si ce n'est PAS une majuscule\n            return (0);        // Retourne 0 immediatement\n        i++;                   // Passe au caractere suivant\n    }\n    return (1);                // Tous les chars sont des majuscules (ou string vide)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putchar(ft_str_is_uppercase(\"HELLO\") + '0');  // 1 : que des majuscules\n    ft_putchar(ft_str_is_uppercase(\"Hello\") + '0');  // 0 : 'e' est minuscule\n    ft_putchar(ft_str_is_uppercase(\"\") + '0');        // 1 : string vide = vrai\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_str_is_uppercase.c -o test && ./test", expectedOutput: "101", explanation: "On ne garde que la plage 'A'-'Z'. Même structure, seules les bornes changent." },
          ...
          { type: "challenge", questions: [
            "Quelle plage ASCII est imprimable ? De 32 (espace) a 126 (~).",
            "Le tab (\\t = 9) est-il imprimable ? Non, 9 < 32.",
          ]},
          { type: "reveal", label: "Voir la solution ft_str_is_printable", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)        // Affiche un seul caractere\n{\n    write(1, &c, 1);           // Ecrit 1 octet sur stdout\n}\n\nint ft_str_is_printable(char *str) // Retourne 1 si que des chars imprimables, 0 sinon\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (!(str[i] >= 32 && str[i] <= 126)) // Si hors plage imprimable (32-126)\n            return (0);        // Retourne 0 immediatement\n        i++;                   // Passe au caractere suivant\n    }\n    return (1);                // Tous les chars sont imprimables (ou string vide)\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    ft_putchar(ft_str_is_printable(\"Hello 42!\") + '0');    // 1 : tous imprimables\n    ft_putchar(ft_str_is_printable(\"Hello\\tWorld\") + '0'); // 0 : '\\t' (9) n'est pas imprimable\n    ft_putchar(ft_str_is_printable(\"\") + '0');              // 1 : string vide = vrai\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_str_is_printable.c -o test && ./test", expectedOutput: "101", explanation: "Caractères imprimables : de l'espace (32) au tilde (126). Le \\t (9), \\n (10) et DEL (127) ne sont PAS imprimables." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "challenge", questions: [
            "Quelle est la difference ASCII entre 'a' et 'A' ? 32.",
            "Comment convertir minuscule en majuscule ? Soustraire 32.",
            "Faut-il toucher aux chiffres et espaces ? Non, seulement les lettres minuscules.",
          ]},
          { type: "reveal", label: "Voir la solution ft_strupcase", code: "#include <unistd.h>\n\nvoid ft_putstr(char *str)      // Affiche une string caractere par caractere\n{\n    while (*str)               // Tant que le caractere n'est pas '\\0'\n        write(1, str++, 1);    // Affiche le char puis avance le pointeur\n}\n\nchar *ft_strupcase(char *str)  // Convertit toute la string en majuscules\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (str[i] >= 'a' && str[i] <= 'z') // Si c'est une minuscule\n            str[i] -= 32;      // Soustrait 32 pour convertir en majuscule ('a' -> 'A')\n        i++;                   // Passe au suivant (meme si pas modifie)\n    }\n    return (str);              // Retourne la string modifiee\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    char s[] = \"Hello 42!\";    // String modifiable (pas un literal)\n\n    ft_putstr(ft_strupcase(s)); // Convertit et affiche \"HELLO 42!\"\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strupcase.c -o test && ./test", expectedOutput: "HELLO 42!", explanation: "On parcourt chaque char. Si c'est une minuscule, on soustrait 32 pour la convertir en majuscule. Les autres chars restent intacts." },
          ...
          { type: "challenge", questions: [
            "C'est l'inverse de strupcase. Que faut-il faire ? Ajouter 32 aux majuscules.",
            "Quel test ? str[i] >= 'A' && str[i] <= 'Z'.",
          ]},
          { type: "reveal", label: "Voir la solution ft_strlowcase", code: "#include <unistd.h>\n\nvoid ft_putstr(char *str)      // Affiche une string caractere par caractere\n{\n    while (*str)               // Tant que le caractere n'est pas '\\0'\n        write(1, str++, 1);    // Affiche le char puis avance le pointeur\n}\n\nchar *ft_strlowcase(char *str) // Convertit toute la string en minuscules\n{\n    int i;\n\n    i = 0;                     // Index de parcours\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (str[i] >= 'A' && str[i] <= 'Z') // Si c'est une majuscule\n            str[i] += 32;      // Ajoute 32 pour convertir en minuscule ('A' -> 'a')\n        i++;                   // Passe au suivant\n    }\n    return (str);              // Retourne la string modifiee\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    char s[] = \"Hello 42!\";    // String modifiable\n\n    ft_putstr(ft_strlowcase(s)); // Convertit et affiche \"hello 42!\"\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strlowcase.c -o test && ./test", expectedOutput: "hello 42!", explanation: "Même pattern inversé : on détecte les majuscules et on ajoute 32. 'A' + 32 = 'a'." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "challenge", questions: [
            "Comment detecter un debut de mot ? Le caractere precedent n'est pas alphanumerique.",
            "Les chiffres font-ils partie du mot ? Oui ! '42hello' → le 'h' n'est PAS un debut de mot.",
            "Quel flag utiliser ? new_word = 1 au debut et apres chaque separateur.",
          ]},
          { type: "reveal", label: "Voir la solution ft_strcapitalize", code: "#include <unistd.h>\n\nvoid ft_putstr(char *str)      // Affiche une string caractere par caractere\n{\n    while (*str)               // Tant que le caractere n'est pas '\\0'\n        write(1, str++, 1);    // Affiche le char puis avance le pointeur\n}\n\nchar *ft_strcapitalize(char *str) // Capitalise le debut de chaque mot\n{\n    int i;\n    int new_word;\n\n    i = 0;                     // Index de parcours\n    new_word = 1;              // Debut de string = debut de mot\n    while (str[i])             // Parcourt chaque caractere\n    {\n        if (str[i] >= 'a' && str[i] <= 'z' && new_word) // Minuscule en debut de mot\n            str[i] -= 32;      // Convertit en majuscule\n        else if (str[i] >= 'A' && str[i] <= 'Z' && !new_word) // Majuscule en milieu de mot\n            str[i] += 32;      // Convertit en minuscule\n        if ((str[i] >= 'a' && str[i] <= 'z')\n            || (str[i] >= 'A' && str[i] <= 'Z')\n            || (str[i] >= '0' && str[i] <= '9')) // Si le char est alphanumerique\n            new_word = 0;      // On est dans un mot, pas un nouveau debut\n        else\n            new_word = 1;      // Separateur : le prochain sera un debut de mot\n        i++;                   // Passe au caractere suivant\n    }\n    return (str);              // Retourne la string modifiee\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    char s[] = \"salut, 42PARIS!monde\"; // String de test avec separateurs\n\n    ft_putstr(ft_strcapitalize(s)); // Affiche \"Salut, 42paris!Monde\"\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strcapitalize.c -o test && ./test", expectedOutput: "Salut, 42paris!Monde", explanation: "Le flag new_word commence à 1 (début = nouveau mot). On capitalise si c'est une lettre en début de mot, on minusculise sinon. Les chiffres font partie du mot sans être capitalisables." },
    ...
    },
    {
      ...
      content: {
        blocks: [
          ...
          { type: "challenge", questions: [
            "Que retourne strlcpy ? La longueur de src (pas de dest).",
            "Que faire si size == 0 ? Ne rien copier, mais retourner strlen(src) quand meme.",
            "Combien de caracteres copier au maximum ? size - 1, pour garder la place du \\0.",
          ]},
          { type: "reveal", label: "Voir la solution ft_strlcpy", code: "#include <unistd.h>\n\nvoid ft_putchar(char c)        // Affiche un seul caractere\n{\n    write(1, &c, 1);           // Ecrit 1 octet sur stdout\n}\n\nvoid ft_putnbr(int nb)         // Affiche un nombre entier\n{\n    if (nb >= 10)              // S'il reste plus d'un chiffre\n        ft_putnbr(nb / 10);   // Appel recursif pour les chiffres de gauche\n    ft_putchar(nb % 10 + '0'); // Affiche le dernier chiffre\n}\n\nvoid ft_putstr(char *str)      // Affiche une string caractere par caractere\n{\n    while (*str)               // Tant que le caractere n'est pas '\\0'\n        write(1, str++, 1);    // Affiche le char puis avance le pointeur\n}\n\nunsigned int ft_strlcpy(char *dest, char *src, unsigned int size) // Copie securisee avec taille max\n{\n    unsigned int i;\n    unsigned int src_len;\n\n    src_len = 0;               // Calcule la longueur de src\n    while (src[src_len])       // Parcourt src jusqu'au '\\0'\n        src_len++;             // Compte chaque caractere\n    if (size == 0)             // Si size vaut 0, on ne copie rien\n        return (src_len);      // Retourne juste la longueur de src\n    i = 0;                     // Index de copie\n    while (src[i] && i < size - 1) // Copie au max size-1 chars (1 place pour '\\0')\n    {\n        dest[i] = src[i];      // Copie le caractere courant\n        i++;                   // Passe au suivant\n    }\n    dest[i] = '\\0';            // Termine TOUJOURS par un '\\0'\n    return (src_len);          // Retourne strlen(src) pour detecter la troncation\n}\n\n// --- Main de test (à retirer avant de rendre) ---\nint main(void)\n{\n    char dest[20];             // Buffer de destination\n    unsigned int ret;          // Valeur de retour\n\n    ret = ft_strlcpy(dest, \"Hello World\", 6); // Copie max 5 chars + '\\0'\n    ft_putstr(dest);           // Affiche \"Hello\" (tronque a 5 chars)\n    write(1, \" \", 1);         // Espace separateur\n    ft_putnbr(ret);            // Affiche 11 (longueur de \"Hello World\")\n    write(1, \"\\n\", 1);        // Retour a la ligne\n    return (0);                // Fin du programme\n}", compileCmd: "gcc -Wall -Wextra -Werror ft_strlcpy.c -o test && ./test", expectedOutput: "Hello 11", explanation: "On calcule strlen(src), puis on copie au max size-1 chars + \\0. Le retour est strlen(src). Si retour >= size → troncation détectable." },
